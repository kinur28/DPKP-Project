...
<div class="container">
	<div class="d-flex align-items-center p-3 my-3 text-dark bg-info rounded shadow-sm">
		...
		<div class="me-2 p-2">
			<form href="{{route('profile')}}"  method="GET" action="/Dashboard/profile">
				<input type="hidden" class="form-control" name="user_id" value="{{auth()->user()->id}}" aria-label="Search">
				<button type="submit" style ="padding: 0; border: none;background: none;"class="nav-link h5 text-dark bg-info"><i class="fas fa-chevron-left"></i>
					Kembali
				</button>
			</form>
		</div>
		<h1 class="h3 mb-0 text-dark ms-5 me-5">Edit Profile</h1>
...
<div class="my-3 mb-3 p-3 bg-body rounded shadow-sm">
	<div class="col-md-7 mx-auto col-lg-8 mb-3">
		...
		<form action="/Dashboard/Edit-Profile/{{$data_karyawan->id}}/update" class="row mt-2 g-3" method="POST" enctype="multipart/form-data">
			{{csrf_field()}}
